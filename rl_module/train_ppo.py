...
def train_ppo(actor_critic, agent, rollouts, task_idx, env_name, task_sequences, envs, new_obs, obs_shape, obs_shape_real, args,
    ...
    for j in tqdm(range(num_updates)):
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1
            ...
            for idx in range(len(task_sequences)):
                te_reward_arr['max']['task' + str(idx)].append((eval_episode_max_rewards[idx]))
                
            for idx in range(len(task_sequences)):
                te_reward_arr['min']['task' + str(idx)].append((eval_episode_min_rewards[idx]))
